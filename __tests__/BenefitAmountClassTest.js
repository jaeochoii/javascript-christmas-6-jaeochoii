...
describe("BenefitAmount 클래스 단위 기능 테스트", () => {
  test("Event 클래스로부터 얻은 혜택 내역을 기반으로 금액으로 환산 되는지 확인(주말)", () => {
  ...
  test("Event 클래스로부터 얻은 혜택 내역을 기반으로 금액으로 환산 되는지 확인(평일, 특별할인 X)", () => {
    const menus = { 티본스테이크: 2, 초코케이크: 1, 제로콜라: 1 };
  ...
  test("Event 클래스로부터 얻은 혜택 내역을 기반으로 금액으로 환산 되는지 확인(평일, 특별할인 O)", () => {
    const menus = { 티본스테이크: 2, 초코케이크: 1, 제로콜라: 1 };
  ...
  test("Event 클래스로부터 얻은 혜택 내역을 기반으로 금액으로 환산 되는지 확인(12만원 미만 결제)", () => {
    const menus = { 티본스테이크: 2, 초코케이크: 1, 제로콜라: 1 };
    const benefitList = [13, 0, 1, 1, 0];
